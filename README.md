...
Response:
{
  "code": 200,
  "data": {
    "sentiment_category": 2,      // 1=正向 2=负向 4=中性
    "sentiment_score": 7.5,       // 强度 0-10
    "sentiment_label": "焦虑",
    "reply": "我感受到你的压力了...",
    "guide": "建议尝试深呼吸练习...",
    "keywords": ["压力", "工作", "疲劳"],
...
🎯 核心模块说明
模块1：情感分析引擎 (backend_core/core/analysis.py)
责任：将用户输入转换为结构化情感数据

输入：原始文本字符串
处理：句向量化 → 情感分类 → 关键词提取
输出：情感类别、强度、标签、建议
模块2：用户认证系统 (routes/auth_route.py)
责任：账户安全管理

密码哈希存储(bcrypt推测)
Session/JWT管理
邮箱唯一性验证
模块3：对话历史管理 (routes/history_route.py)
责任：数据持久化与同步
...
💬 反馈与贡献
欢迎提出Issue和Pull Request！一起让这个项目更温柔。
