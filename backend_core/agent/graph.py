...
async def node_rag_retrieve(state: AgentState) -> AgentState:
    ...
    try:
        ...
        logger.info(
            "[LG] rag_retrieve | context_len=%d refs=%d",
            len(rag_context),
            len(rag_refs),
        )
    except Exception as exc:
    ...
    return {**state, "rag_context": rag_context, "rag_refs": rag_refs}
...
async def run_agent(
    ...
    """
    运行 LangGraph Agent，返回结果字典（与 huawei_nlp_service.analyze_sentiment 相同格式）。
    """
    graph = _get_graph()
    ...
    # ✅把 refs 也挂回 result，让上层统一后处理/统一返回
    ...
    logger.info(
        "[LG] run_agent END | category=%s score=%.1f refs=%d",
        result.get("category"),
        result.get("score", 0),
        len(rag_refs),
    )
